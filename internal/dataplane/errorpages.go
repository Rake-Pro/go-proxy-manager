...
// refuse writes a terminal refusal generated by gpm itself: the configured error
// page for status when one resolves (host override first, then the
// settings-level pages), otherwise the exact plain-text body that gate has
// always written. It is the auth tier's equivalent of the serveErrorPage call
// the access-list, guard, bouncer and rate-limit tiers already make - the same
// resolution rules, the same template context, and byte-identical output when
// nothing is configured.
//
// Only TERMINAL refusals go through here. A redirect into a sign-in flow (OIDC
// 302, the auth-request outpost redirect) is not an error page, and a response
// proxied from the identity provider is the IdP's to own, not gpm's.
func refuse(w http.ResponseWriter, status int, ep *compiledErrorPages, host, body string) {
	serveErrorPage(w, status, ep, host, func() { http.Error(w, body, status) })
}
