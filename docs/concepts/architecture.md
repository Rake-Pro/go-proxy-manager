...
The local admin can carry a **TOTP second factor** (RFC 6238, HMAC-SHA-1, 6
digits, 30-second step, one step of drift either way), enabled by the presence of
`GPM_LOCAL_ADMIN_TOTP_SECRET` (or its `_FILE` form) and implemented on the
standard library alone. A correct password mints no session: it stores a
single-use, five-minute pending entry in the same bounded map the OIDC flow uses
(the two are not interchangeable) and renders a second server-side page asking
for the code. The accepted time step is remembered per user in memory, so a code
is single-use; a wrong code counts toward the same per-client-IP lockout as a
wrong password, and a correct password with the code still outstanding does not
clear that lockout. An unparseable secret is fatal at startup and, defensively,
fails closed in the authenticator rather than silently disabling the factor.

**Roles are scope grants.** `admin` is unconstrained; `user` is `*:read` minus
the `api-tokens` subject, so it reaches every `GET` under `/api/` except the
credential routes and is refused every write. `GET /config` drops `apiTokens`
for any caller lacking `api-tokens:read`, so the exclusion cannot be walked
...
**Docker container discovery** (`internal/docker`). The same subsystem pointed
at containers instead of a cluster: an optional, read-only loop that turns
containers labelled `gpm.rake.pro/enabled: "true"` into gpm-managed proxy hosts,
...
Both reconcilers share one planner (`internal/discovery`): full-state desired
...
record through the referenced DNS provider and waits for propagation against a
...
- **One client IP, three trust tiers.** `internal/dataplane/clientip.go` is the
  only place that answers "what is the client IP", and every IP-comparing tier
  reads its result: access lists, geo, rate limits, the basic-auth lockout, all
  four `allowFrom` exemptions, the access log, and the `X-Forwarded-For` /
