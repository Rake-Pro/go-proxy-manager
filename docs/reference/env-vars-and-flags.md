...
| `GPM_HTTPS_ADDR`<br>`-https-addr` | `:443` | Data-plane HTTPS | Yes |
| `GPM_HTTP_ADDR`<br>`-http-addr` | `:80` | Data-plane HTTP | Yes |
...
| `GPM_SSO_SIGNING_KEY` | (auto-persisted) | HMAC key signing data-plane per-host OIDC session cookies; auto-generated on first use and saved to `<cert-dir>/sso_signing.key` (0600) so sessions survive restarts; set explicitly to supply your own key or share one across instances | Yes |
...
| Command | Flags | Does |
|---|---|---|
| `gpm` | every flag in the table above, plus `-version` | Runs the daemon: data plane, admin plane, ACME, reconcilers. |
