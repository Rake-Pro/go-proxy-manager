...
```
   internet --> +-----------+ -- LAN --> +----------------------+
                | edge host |            | k8s cluster          |
                |  gpm      |            |  ingress controller  |
                |  TLS term |            |  (LB address)        |
                +-----------+            |  Services / Pods     |
                                         +----------------------+
```
...
self-service privilege grant section 7 exists to forbid: `locations: [{path: /,
...
| No privilege inheritance from cluster manifests | every security-relevant field comes from an operator-written profile; the Ingress contributes hostnames (validated, suffix-restricted), two booleans, and the *name* of one profile, never a chain, a middleware, an access list, a certificate or an upstream (section 5a) |
